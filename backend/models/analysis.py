...
from sqlalchemy import String, Text, Integer, DateTime, ForeignKey, CheckConstraint, JSON
...
class Analysis(Base):
    ...
    job_id: Mapped[uuid.UUID] = mapped_column(Uuid(as_uuid=True), ForeignKey("job_descriptions.id", ondelete="CASCADE"), nullable=False)
    parent_id: Mapped[uuid.UUID | None] = mapped_column(Uuid(as_uuid=True), ForeignKey("analyses.id", ondelete="SET NULL"), nullable=True)
